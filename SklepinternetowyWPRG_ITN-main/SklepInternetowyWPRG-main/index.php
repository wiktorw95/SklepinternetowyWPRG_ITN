...
if (isset($_SESSION['recently_viewed']) && is_array($_SESSION['recently_viewed']) && count($_SESSION['recently_viewed']) > 0) {
    $recently_viewed = $_SESSION['recently_viewed'];
    $placeholders = implode(',', array_fill(0, count($recently_viewed), '?'));
    $types = str_repeat('i', count($recently_viewed));

    $stmt = $conn1->prepare("SELECT * FROM products WHERE product_id IN ($placeholders)");
    $stmt->bind_param($types, ...$recently_viewed);
    $stmt->execute();
    $result = $stmt->get_result();
    $recent_products_unordered = $result->fetch_all(MYSQLI_ASSOC);

    $recent_products = [];
    foreach ($recently_viewed as $id) {
        foreach ($recent_products_unordered as $product) {
